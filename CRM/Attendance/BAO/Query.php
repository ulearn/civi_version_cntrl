...
class CRM_Attendance_BAO_Query 
{
	...
	static function getAllGroup() 
	{
			...
			$options = array();
			$query = "SELECT name, id FROM civicrm_group order by name";
	...
	static function buildSearchForm( &$form ) 
    {
        ...
        $searchOptions = CRM_Core_BAO_Preferences::valueOptions( 'advanced_search_options' );
		  ...
		  if ( CRM_Utils_Array::value( 'groups', $searchOptions ) ) {
            ...
            if ( $config->groupTree ) {
                ...
                $form->assign('groupIds', implode( ',', $groupIds ) );
                $form->assign('groupNames', $groupNames );
            } else {
                // add select for groups
               // $group = CRM_Attendance_BAO_Query::getAllGroup();
			   //	$options = array();
				...
				while ( $dao12->fetch()) {
				 ...
				 $dao12->free();
			   	 //$group = '';
                 $form->addElement('select', 'group', ts('Select Group'), $options, array('selected'=>$_REQUEST['group']));
            }
        }
       ...
       // $form->addDate( 'member_join_date_low', ts('Join Date - From'), false, array( 'formatType' => 'searchDate') );
        ...
        $form->addDate( 'member_start_date_low', ts('Start Date'), false, array( 'formatType' => 'searchDate', 'value' => $_REQUEST['member_start_date_low']) );
        //$form->addDate( 'member_start_date_high', ts('To'), false, array( 'formatType' => 'searchDate') );

        $form->addDate( 'member_end_date_low', ts('End Date'), false, array( 'formatType' => 'searchDate', 'value' => $_REQUEST['member_end_date_low']) );
