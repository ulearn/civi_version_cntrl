...
class CRM_Attendance_Form_Search extends CRM_Core_Form 
{
    ...
    { 
		...
		if((isset($_REQUEST['outstanding_amount']) && $_REQUEST['outstanding_amount'] == 'Outstanding Amount') || (isset($_GET['outstanding_amount']) && $_GET['outstanding_amount'] == 'Outstanding Amount') )
			{
				...
				if(isset($_REQUEST['member_start_date_low']))
				{
					$member_start_date_low = $_REQUEST['member_start_date_low'];
				...
				if(isset($_REQUEST['member_end_date_low']))
				{
					$member_end_date_low = $_REQUEST['member_end_date_low'];
				...
				/*if(!empty($_POST['group']) && !empty($_POST['member_start_date_low'])  && !empty($_POST['member_end_date_low']))
				 ...
				 if(!empty($_REQUEST['member_start_date_low'])){
				 	$and_str.=" AND civicrm_membership.start_date >= '$member_start_date_low' ";
				 }
				 if(!empty($_REQUEST['member_end_date_low'])){
		...
		if(isset($_REQUEST['force_to_100']) && $_REQUEST['force_to_100'] == 'Force To 100 Percent')
			{
				$_REQUEST['view_partial_attendance'] = 'View Partial Attendance Report';
				$_REQUEST['group'] =  $_REQUEST['hide_group'];
				$row_count = $_REQUEST["force_to_100_count"];
				for ($row_loop_start = 1 ; $row_loop_start <= $row_count ; $row_loop_start++)
				{
					$key_to_check = 'selectbox_contact_'.$row_loop_start;
					if(isset($_REQUEST["$key_to_check"]) && !empty($_REQUEST["$key_to_check"]))
					{
						$id_to_update = $_REQUEST["$key_to_check"];
		...
		if(isset($_REQUEST['group']) &&  isset($_REQUEST['export_attendance']) && !empty($_REQUEST['member_start_date_low']) && !empty($_REQUEST['member_end_date_low']))
		{
			$group_id = $_REQUEST['group'];
			$member_start_date_low = $_REQUEST['member_start_date_low'];
			$member_start_date_low = date('Y-m-d', strtotime($member_start_date_low));
			...
			while ( $select_query_result->fetch()) 
			{
        		//checking paid or not
				$paid_status1 = false;
				$select_query_paid_check = "SELECT DISTINCT contact_id FROM  civicrm_contribution where contact_id = '$select_query_result->id' AND contribution_status_id NOT IN(3,4)";
		...
		if(isset($_REQUEST['group']) &&  isset($_REQUEST['_qf_Search_submit']) && !empty($_REQUEST['member_start_date_low']) && !empty($_REQUEST['member_end_date_low']))
		{
			$group_id = $_REQUEST['group'];
			$member_start_date_low = $_REQUEST['member_start_date_low'];
			$member_start_date_low = date('Y-m-d', strtotime($member_start_date_low));
