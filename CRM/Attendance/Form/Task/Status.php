 ...
 */
class CRM_Attendance_Form_Task_Status extends CRM_Attendance_Form_Task {
   ...
   /* function buildQuickForm( ) 
	...
	function buildQuickForm()
	{
			 ...
			 $query2 = "SELECT name, id FROM civicrm_group order by name";
				$dao12 = &CRM_Core_DAO::executeQuery( $query2 );
				$i = 0;
				$options  = array('' => ts('- any group -'));
				while ( $dao12->fetch()) {
					$options[$dao12->id] = $dao12->name;
					++$i;
				}
				 $dao12->free();
			   	 //$group = '';
                 //$form->addElement('select', 'group', ts('Select Group'), $options);
			 
			// $this->assign( 'options', $options );
		$this->addElement('select', 'group', ts('Select Group'), $options);
			 
		$this->addDate( 'member_join_date_low', ts('Join Date - From'), false, array( 'formatType' => 'searchDate') );
        $this->addDate( 'member_join_date_high', ts('To'), false, array( 'formatType' => 'searchDate') );

        $this->addDate( 'member_start_date_low', ts('Start Date - From'), false, array( 'formatType' => 'searchDate') );
        $this->addDate( 'member_start_date_high', ts('To'), false, array( 'formatType' => 'searchDate') );

        $this->addDate( 'member_end_date_low', ts('End Date - From'), false, array( 'formatType' => 'searchDate') );
        $this->addDate( 'member_end_date_high', ts('To'), false, array( 'formatType' => 'searchDate') );
