...
echo "\n\nInitiating Synchronisation with Mailchimp ...\n\n";
...
$syncrun->removeContactsRemoved();

$syncrun->removeContactsDeleted();

$syncrun->addWebhooks();

$syncrun->addContactsSubscribed();

$syncrun->getBulkActivity();
...
class mailchimpSync {
	 ...
	 */
	function __construct($debugging = 'off', $display_errors = 0) {
		...
		if (!file_exists($configLocation)) $configLocation = str_replace("bin/mailchimp_sync.php", "civicrm.config.php", __FILE__); // Drupal 
		...
		list($db_username, $db_password) = explode(':', str_replace("mysqli://", "", $db_user_details));
		list($db_server, $db_database) = explode('/', str_replace("?new_link=true", "", $db_access_details));
		
		$this->apikey = (isset($_GET['apikey'])) ? $_GET['apikey'] : "81f29a2d01d1645177ccd1b28472a479-us4";
		...
		if (isset($_POST) && !empty($_POST)) $this->handleNotification($_POST);
	...
	public function createTables() {
		...
		echo "\n\nCreating sync log tables if they don't alredy exist ...\n\n";
		...
		// TODO: throw error if we don't have create privileges on db
		$this->verbose();
	}
	
	/*
	 * handles request from webhooks that are sent from mailchimp, events are bounces and unsubscribes.
	 */
	public function handleNotification($data) {
			...
			$sql = "INSERT INTO temp_table (content) VALUES ('$buffer')";
			$result = mysql_query($sql);
	...
	public function addGroups() {
		...
		$sql = "SELECT id, name, title FROM civicrm_group WHERE group_type = '2' AND is_active = '1'";
	...
	public function addContacts() {
		...
		// Add contacts that are in CiviCRM Groups to MailChimp (that have been added from CiviCRM groups)
		...
		$sql = "SELECT  
				...
				civicrm_email.email,
				GROUP_CONCAT(civicrm_group.title SEPARATOR ',') grouplist
		...
		while ($contact = mysql_fetch_array($group_list)) {
			
			if(empty($contact['grouplist'])) continue;
			
			$groups = explode(',', $contact['grouplist']);
			...
			foreach ($groups as $grp) $buffer['GROUPINGS'][] = array('name'=>$grp, 'groups'=>'default');
		...
		subscribed. Mailchimp recommend you not attempt to add more than 5000-10,000 
		addresses per call to listBatchSubscribe()
		*/
		
		if (!empty($batch)) $result = $this->api->listBatchSubscribe($this->list_id, $batch, false, true, true);
		
		if (isset($result)) echo "\n\nGroups and Contacts Successfully added to Mailchimp...\n\n";
		
		else if (!empty($batch)) echo "\n\nThere was an error updating contacts with Mailchimp.\n\n";
		
		if (isset($result)) print_r($result);
		
		if (isset($result)) {
		
			echo "added   : ".$result['add_count']."\n";
			echo "updated : ".$result['update_count']."\n";
			echo "errors : ".$result['error_count']."\n";
			
			if (!empty($result['errors'])) {
				foreach($result['errors'] as $err){
					echo $err['code'].' - '.$err['message']."\n";
					if (isset($err['row'])) echo "\t".$err['row']['email']."\n";
				}
			}
		
		}
		
		echo "\n\nRemoving subscribe users from log table\n\n";
	...
	/*
	 * Remove contacts from Mailchimp groups that were removed from corresponding CiviCRM groups
	 */
	public function removeContactsRemoved() {
		
		echo "\n\nRemoving contacts from Mailchimp groups that were removed from corresponding CiviCRM groups ...\n";
		
		$sql = "SELECT civicrm_email.contact_id, IFNULL(civicrm_contact.first_name, civicrm_contact.display_name) contact_name, civicrm_email.email, GROUP_CONCAT(civicrm_group.title SEPARATOR ',') grouplist 
				FROM civicrm_group_contact
				LEFT JOIN civicrm_email ON civicrm_email.contact_id = civicrm_group_contact.contact_id
				LEFT JOIN civicrm_group ON civicrm_group.id = civicrm_group_contact.group_id AND civicrm_group_contact.status = 'Removed'
				LEFT JOIN civicrm_contact ON civicrm_contact.id = civicrm_email.contact_id  
				WHERE civicrm_group_contact.status = 'Removed'
				AND civicrm_email.email NOT IN (SELECT email FROM civicrm_mailchimp_sync_log_removes)
				GROUP BY civicrm_email.email";
		...
		if (!$result) print_r(mysql_error());
			...
			while ($contact = mysql_fetch_assoc($result)) {
				...
				$merge_vars = array('FNAME' => $contact['contact_name'], 'GROUPINGS' => array());
				
				//if (strlen($contact['grouplist']) > 0) 
				foreach ($groups as $grp) $merge_vars['GROUPINGS'][] = array('name'=>$grp, 'groups'=>'');
				...
				if (isset($contact['email'])) {
					...
					$retval = $this->api->listUpdateMember($this->list_id, $contact['email'], $merge_vars, '', true);
					...
					if ($this->api->errorCode) {
						...
						$sql = "REPLACE INTO civicrm_mailchimp_sync_log_removes (email) VALUES ('{$contact['email']}')";
						mysql_query($sql);
	...
	public function removeContactsDeleted() {
		...
		echo "\n\nRemoving contacts from Mailchimp that were deleted from CiviCRM ...\n";
		...
		if (!$result) print_r(mysql_error());
			...
			while ($contact = mysql_fetch_assoc($result)) $emails[] = $contact['email'];
			
			if (!empty($emails)) {
			
				if ($this->debugging == 'on') print_r($emails);
				...
				$sql = "REPLACE INTO civicrm_mailchimp_sync_log_removes (email) VALUES ('".implode("','", $emails)."')";
				mysql_query($sql);
	...
	public function addWebhooks() {
		...
		echo "\n\nAdding web hook for this script, at the this address:\n\n$notify_url\n\n";
		
		// TODO: include the actions parameter so that we only get notification of unsubscribes, and cleaned emails (bounces)
		// see: http://apidocs.mailchimp.com/api/1.3/listwebhookadd.func.php
		
		$result = $this->api->listWebhookAdd($this->list_id, $notify_url);
		
		// neeed to include this line for webhook call to work
		print_r($result);
		$this->verbose();
	}
	
	/*
	 * Record emails sent via Mailchimp as an activity in each corresponding CiviCRM contact that the emails were sent to
	 */
	public function getBulkActivity() {
		
		echo "Getting bulk activity records for all users that are in CiviCRM mailing list groups ...\n\n";
		
		// first of all get a list of emails that are in groups
		
		$sql = "SELECT email FROM civicrm_email WHERE contact_id IN 
				(SELECT civicrm_group_contact.contact_id 
				FROM civicrm_group_contact 
				LEFT JOIN civicrm_group ON civicrm_group.id = civicrm_group_contact.group_id 
				WHERE civicrm_group.group_type = '2')
				AND is_primary = 1";
		...
		while ($item = mysql_fetch_array($result)) $email_list[] = $item['email'];
		
		// split the list into chunks of 50 - that is the maximum number you can query in API call
		
		$emailArray = array_chunk($email_list, 50);
		
		if ($this->debugging == 'on') print_r($emailArray);
		
		$activity_dump = array();
		
		foreach ($emailArray as $emailList) {
			$activity_dump[] =  $this->api->listMemberActivity($this->list_id, $emailList);	
		}
		
		if (empty($activity_dump[0])) {
			mysql_close($this->con);
			die('No activity data returned from Mailchimp - exiting ...');
		}
		
		if ($this->debugging == 'on') print_r($activity_dump);
		
		for ($n=0; $n<count($emailArray); $n++) {
			
			$listSize = count($emailArray[$n]);
			
			for ($i=0; $i<$listSize; $i++) {
			
				foreach ($activity_dump[$n]['data'][$i] as $activity) {
					
					echo $activity['action']."<br />";
					
					// only record activities relating to email being sent / bounced
					//if (in_array($activity['action'], array('click','abuse','open','unsub'))) continue;
					if ($activity['action'] != 'sent') continue;
					
					// get contact's id based on their email address
				
					$sql = "SELECT contact_id FROM civicrm_email WHERE email = '{$emailArray[$n][$i]}'";
					
					$result = mysql_query($sql);
					$contact_id = mysql_result($result, 0); // there should only be one as email addresses are unique
					
					// get campaign name from id
					$campaign_data = $this->api->campaigns(array("campaign_id" => $activity['campaign_id']));
					
					$campaign_name = $campaign_data['data'][0]['title'];
					
					// record activity
					
					$details = (isset($activity['campaign_id'])) ? "Included {$emailArray[$n][$i]} in Mailchimp Email Campaign: ".$campaign_name : "";
					
					$status = ($activity['action'] == "sent") ? 2 : 5;
					
					// first see if this record has added already ..
					
					$sql = "SELECT id FROM civicrm_activity WHERE activity_date_time = '{$activity['timestamp']}' AND subject = '{$activity['action']}' AND details = '$details' AND status_id = $status";
					
					if ($this->debugging == 'on') echo $sql."\n\n";
					...
					if (!mysql_num_rows($result)) { 
						...
						$sql = "INSERT INTO civicrm_activity (source_contact_id, source_record_id, activity_type_id, subject, activity_date_time, duration, location, phone_id, phone_number, details, status_id, priority_id, parent_id, is_test, medium_id, is_auto, relationship_id, is_current_revision, original_id, result, is_deleted, campaign_id, engagement_level) VALUES
								(1, NULL, 3, '{$activity['action']}', '{$activity['timestamp']}', NULL, NULL, NULL, NULL, '$details', $status, 2, NULL, 0, NULL, 0, NULL, 1, NULL, NULL, 0, NULL, NULL)";
	...
	public function updateGroupNames() {
		...
		$mailchimp_groups = $this->api->listInterestGroupings($this->list_id);
		
		if ($this->debugging == 'on') print_r($mailchimp_groups);
		
		while($group = mysql_fetch_assoc($result)) {
			foreach ($mailchimp_groups as $mgrp) if ($mgrp['name'] == $group['old_name']) $group_id = $mgrp['id'];
	...
	public function updateContactNames() {
		...
		while($contact = mysql_fetch_assoc($result)) {
			...
			if ($this->debugging == 'on') print_r($contact);
	...
	/*
	 * Now get a list of all members who have been updated on Mailchimp, and update their groups on CiviCRM (Bug #790)
	 */
	public function updateContactsUpdated() {
		
		echo "Running Function\n\n";
		
		$new_member_list = $this->api->listMembers($this->list_id, 'updated', date('Y-m-d H:i:s', strtotime('-1 day')));
		
		echo "\n\nUpdating CiviCRM contacts that were updated on Mailchimp ...\n\n";
		
		$new_member_emails = array();
		...
		if ($this->api->errorCode) {
			...
			echo "Members matched: ". $new_member_list['total']. "\n";
			echo "Members returned: ". sizeof($new_member_list['data']). "\n";
			foreach($new_member_list['data'] as $member){
			    echo $member['email']." - ".$member['timestamp']."\n";
			    $new_member_emails[] = $member['email']; 
			}
		}
		
		$updated_member_data = $this->api->listMemberInfo($this->list_id, $new_member_emails);
		
		if ($this->debugging == 'on') print_r($updated_member_data);
		
		$last_contact_id = 'notset';
			
		if (isset($updated_member_data['data']) && !empty($updated_member_data['data'])) {
			foreach ($updated_member_data['data'] as $new_member) {
				...
				foreach ($new_member['merges']['GROUPINGS'] as $group) if (isset($group['groups']) && strlen($group['groups']) > 0) $groups[] = $group['name'];
				
				// FIRST: DETERMINE IF CONTACT ALREADY EXISTS IN CIVICRM
				...
				while ($row = mysql_fetch_array($result)) $contact_id = $row['contact_id'];
				
				// this just means that the email address wasn't found for this contact (they are a new contact - added via mailchimp)
				if ($last_contact_id == $contact_id && $last_contact_id != 'notset') continue;
				else $last_contact_id = $contact_id;
				
				echo "\n\ncontact id: ".$contact_id;
				
				if (!$contact_id || empty($contact_id)) continue;
				
				// update the contact's names
				$sql = "UPDATE civicrm_contact SET display_name = '$displayname', sort_name = '$sortname', first_name = '$firstname'";
				...
				foreach ($groups as $grp) {
					...
					$sql = "SELECT id FROM civicrm_group WHERE title = '$grp'";
	...
	/*
	 * Now get a list of all members who have subscribed to the CiviCRM mailing list (via Mailchimp) since last time script ran (assuming it runs daily).
	 */
	public function addContactsSubscribed() {
		
		$new_member_list = $this->api->listMembers($this->list_id, 'subscribed', date('Y-m-d H:i:s', strtotime('-1 day')));
		
		echo "\n\nAdding users who subscribe on Mailchimp as CiviCRM contact ...\n\n";
		...
		$new_member_data = $this->api->listMemberInfo($this->list_id, $new_member_emails);
		
		if ($this->debugging == 'on') print_r($new_member_data);
			
		if (isset($new_member_data['data']) && !empty($new_member_data['data'])) {
			foreach ($new_member_data['data'] as $new_member) {
				$firstname = $new_member['merges']['FNAME'];
				$lastname = (isset($new_member['merges']['LNAME'])) ? $new_member['merges']['LNAME'] : null;
				$email = $new_member['merges']['EMAIL'];
				...
				foreach ($new_member['merges']['GROUPINGS'] as $group) $groups[] = $group['name'];
				...
				if (!$data || empty($data)) {
					...
					echo "\n\n\nAdded to CiviCRM: $sortname\n\n\n";
				}
				else {
					echo "\n\nA contact with that email address already exists: $email\n\n";
				}	
			}
		}
		$this->verbose();
